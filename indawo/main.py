...
async def switch_to_auto_mode():
    ...
    mqtt.CLIENT.publish_mode('AUTO_MODE')
...
def button_trigger_callback(button):
    ...
    if not MANUAL_CONTROL:
        ...
        mqtt.CLIENT.publish_mode('MANUAL_MODE')
        MANUAL_CONTROL = True
...
def start_service():
    ...
    mqtt.CLIENT.publish_light(lights.MAIN_LIGHT)
    mqtt.CLIENT.publish_light(lights.BASKING_LIGHT)
    mqtt.CLIENT.publish_light(lights.NIGHT_LIGHT)
